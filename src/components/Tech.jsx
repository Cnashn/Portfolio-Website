...
const TechTile = ({ tech, index }) => (
  <motion.div
    variants={tileVariant(index)}
    whileHover={{ y: -6 }}
    transition={{ type: "spring", stiffness: 320, damping: 20 }}
    className="w-full"
  >
    <BackgroundGradient animate={false} containerClassName="rounded-2xl p-[1.5px]">
    <SpotlightCard
      className="group aspect-square rounded-2xl bg-tertiary/90 border border-white/10 hover:border-[#1cb9d7]/40 hover:shadow-[0_8px_32px_rgba(28,185,215,0.12)] transition-[border-color,box-shadow] duration-300 flex flex-col items-center justify-center gap-2 sm:gap-3 p-3"
    >
      <img
        src={tech.icon}
        alt={tech.name}
